package com.cloudshi.demo.zxing.utils;
...
public class QRBarCodeUtil {
    ...
    public static void createCodeToFile(String codeContent, File saveDir, String fileName) {
        try {
            ...
            if (null==saveDir || saveDir.isFile()) {
            ...
            if (StringUtils.isBlank(fileName)) {
                fileName = LocalDateTime.now().toInstant(ZoneOffset.of("+8")).toEpochMilli() + ".png";
                System.out.println("二维码图片文件名为空，随机生成 png 格式图片...");
            }

            /**com.google.zxing.EncodeHintType：编码提示类型
             * EncodeHintType.CHARACTER_SET：设置字符编码类型
             * EncodeHintType.ERROR_CORRECTION：设置误差校正, ErrorCorrectionLevel：误差校正等级
             ...
             */
            File codeImgFile = new File(saveDir, fileName);
    ...
    public static void createCodeToOutputStream(String codeContent, OutputStream outputStream) {
        try {
            ...
            /**com.google.zxing.EncodeHintType：编码提示类型
             * EncodeHintType.CHARACTER_SET：设置字符编码类型
             * EncodeHintType.ERROR_CORRECTION：设置误差校正，等级不一样，生成的图案不同，但扫描的结果是一样的
             * EncodeHintType.MARGIN：设置二维码边距，单位像素，值越小，二维码距离四周越近
             */
            Map<EncodeHintType, Object> hints = Maps.newHashMap();
            hints.put(EncodeHintType.CHARACTER_SET, "UTF-8");
    ...
    public static String parseQRCodeByFile(File file) {
        ...
        try {
             ...
             * 如果图片路径错误，比如不能访问，则 read 抛异常：javax.imageio.IIOException
             */
    ...
    public static String parseQRCodeByUrl(URL url) {
        ...
        try {
            /**ImageIO 的 BufferedImage read(URL input) 方法用于读取网络图片文件转为内存缓冲图像
             * 同理还有：read(File input)、read(InputStream input)、read(ImageInputStream stream)
             * 如果图片网络地址错误，比如不能访问，则 read 抛异常：javax.imageio.IIOException
             */
            ...
            BufferedImageLuminanceSource source = new BufferedImageLuminanceSource(bufferedImage);
            BinaryBitmap bitmap = new BinaryBitmap(new HybridBinarizer(source));
            HashMap<DecodeHintType, Object> hints =Maps.newHashMap();
            hints.put(DecodeHintType.CHARACTER_SET, "UTF-8");
